...
// Environment overrides let a fork route policy at its own repository
// without editing the checked-in config (e.g. ISSUE_REPO=jianxx/deepseek-harness).
const [ENV_ORG, ENV_REPO] = (process.env.ISSUE_REPO ?? '').split('/')
const ORG = ENV_ORG || config.organization
const REPO = ENV_REPO || config.repository
...
async function issueSnapshot(number, status = undefined) {
  const issue = await api(`/repos/${ORG}/${REPO}/issues/${number}`)
  if (issue.pull_request) return null
  const values = await api(
    `/repos/${ORG}/${REPO}/issues/${number}/issue-field-values?per_page=100`,
  )
...
async function upsertAudit(number, errors) {
  ...
  if (errors.length === 0) {
    if (existing) {
      await api(`/repos/${ORG}/${REPO}/issues/comments/${existing.id}`, {
        method: 'DELETE',
      })
    }
    return
  ...
  if (existing) {
    ...
    await api(`/repos/${ORG}/${REPO}/issues/${number}/comments`, {
      method: 'POST',
      body: JSON.stringify({ body }),
      headers: { 'Content-Type': 'application/json' },
    })
...
async function pullRequestSnapshot(number) {
  const [pull, reviewRequests, reviews] = await Promise.all([
    api(`/repos/${ORG}/${REPO}/pulls/${number}`),
    api(`/repos/${ORG}/${REPO}/pulls/${number}/requested_reviewers`),
    api(`/repos/${ORG}/${REPO}/pulls/${number}/reviews?per_page=100`),
  ])
...
async function lifecyclePullRequestSnapshot(number) {
  const pull = await api(`/repos/${ORG}/${REPO}/pulls/${number}`)
