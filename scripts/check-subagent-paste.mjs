#!/usr/bin/env node
/**
 * check-subagent-paste.mjs — Stop hook that flags when the assistant pastes a
 * subagent's output verbatim (a ≥40-line quoted block carrying agent-contract
 * headers) instead of distilling it — a violation of the AGENTS.md
 * context-discipline rule.
 *
 * Registered in .claude/settings.local.json (user-local, gitignored).
 *
 ...
 *   - Fail-closed: any parse/read/missing-field error → silent exit 0; a broken
 *     hook must never block the workflow.
 *   - In strict mode, bail when stdin.stop_hook_active === true to avoid the
 *     8× block-cap loop.
 *
...
const SYSTEM_MSG =
  '⚠️ Suspected wholesale paste of subagent output (violates the AGENTS.md context-discipline rule) — distill conclusions instead of relaying the block.'
const BLOCK_REASON =
  'Suspected wholesale paste of subagent output (violates the AGENTS.md context-discipline rule). Synthesize conclusions; do not relay the subagent response verbatim.'
...
//    Strip the '>' quote marker before scanning content for contract headers,
//    so anchored header regexes still match '> Changed:' etc.
let i = 0
