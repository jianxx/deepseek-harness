...
```ts
import PermissionRules from '@deepseek-ai/dsh-permission-rules'
...
`@deepseek-ai/dsh-permission-rules/invariant` rejects any `permission/mode` session event whose value is outside the closed `PERMISSION_MODES` vocabulary.
