...
import ToolRuntime, { defineContentToolFixture, type ToolExecutionInput, type ToolExecutionResult } from '@deepseek-ai/dsh-tools'
...
describe('foldPermissionMode', () => {
  it('folds the last recorded mode', () => {
    const session = Session.create(SessionId('m'))
    session.append('permission/mode', { mode: 'acceptEdits' })
    session.append('permission/mode', { mode: 'bypassPermissions' })
    expect(foldPermissionMode(session.events)).toBe('bypassPermissions')
    expect(foldPermissionMode(Session.create(SessionId('x')).events)).toBeUndefined()
  })
})
