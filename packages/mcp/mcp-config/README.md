# @deepseek-ai/dsh-mcp-config

English | [中文](README.zh.md)

Claude Code-style MCP workspace-configuration loader: parses a `.mcp.json` document, validates it, expands environment substitutions, applies an enterprise allow/deny policy, and translates the accepted servers into `@deepseek-ai/dsh-mcp-client` registrations ready to mount.

This package owns the file→config *reading and validation* surface only. It performs no network I/O and mounts nothing; consumers feed its output to `@deepseek-ai/dsh-mcp-client` instances.

## Usage

```ts
import { buildRegistrations, type McpConfigPolicy } from '@deepseek-ai/dsh-mcp-config'
...
Each accepted server translates to one `@deepseek-ai/dsh-mcp-client` `Config`:
