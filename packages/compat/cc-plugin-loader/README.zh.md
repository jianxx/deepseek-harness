...
这个兼容性加载器读取 CC 插件清单子集，用 [`@deepseek-ai/dsh-skill-claude-code`](../../skill/skill-claude-code/README.md) 与 [`@deepseek-ai/dsh-claude-code-agents`](../../preset/claude-code-agents/README.md) 的纯函数翻译每个组件，并通过 `ctx.get(...)` 读取该组件的宿主 seam。它不是运行时：它产出类型化挂载与结构化报告，把执行交给它注册到的 seam。
