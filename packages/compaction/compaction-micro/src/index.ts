 ...
 * pass over unchanged history emits a byte-identical prompt — and recorded in a
 * durable `compaction/microcompact` event so it is reconstructable from the log.
 *
 * @module @deepseek-ai/dsh-compaction-micro
 */
...
// Type-only: the `compaction/*` SessionEventMap merges (the microcompact record and shadow-price event).
...
export class Microcompactor extends Service {
  ...
  microcompactSession(session: Session): MicrocompactResult {
    ...
    for (const candidate of candidates.slice(0, retainedFrom)) {
      ...
      // Decision record, log-only: names the full-fidelity source each
      // replacement cites, its call, and the re-embedded spill locator. The
      // replacement content itself already carries the deterministic marker, so
      // the decision reconstructs from replay + code even without this event;
      // this record makes the window/freeze policy readable by a pure consumer.
      session.append('compaction/microcompact', {
        originalSeq: seq,
